...
void BookController::update(const HttpRequestPtr& req,
    ...
    Reader item;
        item.id = (*body)["id"].asInt();
        item.isReading = (*body)["isReading"].asBool();
    
    {
         auto re = service_->update(item);
            if (re.isFail())
            {
                    Json::Value err;
                    err["error"] = re.error();
    ...
    auto libro = service_->getById(item.id);
    auto resp = HttpResponse::newHttpJsonResponse(libroToJson(libro.value()));
    resp->setStatusCode(k201Created);
    cb(resp);
    }
}
